<template>
    <div v-if="init" :key="galleryID">
        <div v-if="$attrs.gallery" class="moka-components-gallery grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 md:grid-flow-cols md:gap-8 lg:grid.cols-4 w-full items-start justify-start cursor-pointer object-fit" style="grid-auto-rows: .5fr;">
            ...
            <div v-for="(comp,c) in objects" class="mb-2 text-sm border  shadow-lg rounded-lg justify-center flex flex-col h-full" :title="comp.description + ' ' + $moment(comp.updated_at)">
                <div class="flex-1 relative object-cover ">
                    ...
                    <div class="px-1 absolute bottom-0 rounded-br rounded-bl bg-gray-300 items-center flex flex-row w-full text-left text-xs justify-between" @mouseenter="overID=comp.id" @mouseleave="overID=null">
                        <div class="flex flex-col w-full relative">
                            <div :class="'w-full absolute top-0 left-0 -mt-6 border-b-0 pb-1 pt-1 bg-white text-sm border rounded-tr rounded-tl justify-around flex flex-row ' + showOptions(comp.id)">
                                <i class="ml-2 material-icons text-gray-500 hover:text-blue-500" title="Restore last autosave" @click="restoreAutosave(comp)">medication</i>
                            ...
                            <div class="w-full p-1 flex flex-row justify-around">
                                <i class="material-icons xs mr-2 hover:text-blue-500" title="Edit" @click="selectComponent(comp.id,'component')" @mouseleave="overID=comp.id" @mouseenter="overID=null">edit</i>
...
export default {
    ...
    methods:{
        ...
        setAutosavePreview(){
            ...
            var w = window.open(route.href, 'moka');
        },
