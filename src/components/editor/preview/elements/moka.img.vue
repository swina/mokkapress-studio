<template>
    <img :ref="el.id" :src="hasImage" :caption="el.image && el.image.name ? el.image.caption||el.image.name : 'no-image'" :alt="el.image ? el.image.alternative_text||el.image.name : 'no-image'" :title="el.image && el.image.name ? el.image.name.split('.')[0] : 'no-image'" :class="$cssResponsive(el.css)"/>
</template>
