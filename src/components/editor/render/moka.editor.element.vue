<template>
    <div editorelement v-if="element" :class="$attrs.develop ? 'relative ' : ''" @dblclick="$emit('editinline',element)" :style="element.style">
            ...
            <img v-if="el.element === 'img' && el.image && el.image.url && el.image.ext != '.svg'" 
                :src="$imageURL(el.image)" :caption="el.image.caption" :alt="el.image.alternative_text" :class="$cssResponsive(el.css) + imageCSSDefault" :style="el.css?'':'max-width:10rem;'"/>
            ...
            <img :class="$cssResponsive(el.css) + imageCSSDefault" :ref="el.id" v-if="el.type==='video' && el.image && el.image.url && el.image.ext != '.svg'" :style="el.css?'':'max-width:2rem;'" :src="$imagePreviewURL(el.image)"/><!--el.image.previewUrl-->
            ...
            <div v-if="el.type==='plugin' && typeof el.plugin === 'object'" :class="el.css" :style="el.style">
                ...
                <moka-plugin-wrapper v-if="el.plugin.editor"  :block="el"  :plugin="el" :name="el.plugin.component"/>
            </div>
            ...
            <div v-if="$attrs.develop " :class="'absolute border border-green-500 border-dashed top-0 left-0 bottom-0 right-0 scale-x-100 scale-y-100 transform z-' + $attrs.level + ' ' + active(el.id,el.css) + ' bg-transparent'" style="min-height:2rem" @click="select(el)" @contextmenu="$contextMenu($event,el,el.element==='menu'?'edit':'customize')">
             ...
             <div v-if="el.id===moka.selected" class="z-top absolute top-0 left-0 -mt-6 h-6 bg-gray-800 text-gray-300 text-xs rounded-2xl items-center flex flex-row">
                    ...
                    <i class="material-icons text-sm text-gray-600 hover:text-blue-500 leading-4 mr-2" @click="toolbar=!toolbar" v-if="toolbar">arrow_left</i>
                        ...
                        <i v-if="el.type==='plugin'" class="material-icons hover:text-blue-500 text-sm leading-4 mx-2" @click="$action('pluginsetting')" title="Plugin Settings">settings</i>
                        <i class="material-icons hover:text-blue-500 text-sm leading-4 mx-2" @click="$action('edit')" title="Edit content">edit</i>
