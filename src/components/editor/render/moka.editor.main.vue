            ...
            <div :class="'relative mt-2 mx-2 p-2 cursor-pointer'">
                ...
                <div :class="'fixed z-xtop top-0 left-0 bg-gray-500 p-1 grid grid-cols-3 items-center w-full m-auto cursor-pointer'" title="Select component body">
                    ...
                    <div class="absolute right-0 mr-2 flex flex-row items-center">
                        ...
                        <i class="material-icons hover:text-blue-500 mr-2" @click="$action('addreusable')" title="Import component">system_update_alt</i>
                        ...
                        <i class="material-icons hover:text-blue-500 mr-2" v-if="doc && doc.hasOwnProperty('slider')" title="Slider Settings" @click="sliderSettings=!sliderSettings,disable=false">settings</i>
                    </div>
                </div>
                
                <!-- BLOCKS MAIN EDITOR -->
                <div :class="'pb-10 mt-4 mb-10 mx-5 ' + pageCss">
 ...
 title="Save document">save</i>
            ...
            <div class="p-2 bg-gray-200 rounded">
                ...
                <div v-if="bodySettings">
                    ...
                    <select class="w-full" v-model="fontFamily">
                        ...
                        <option v-for="font in moka.fonts" :value="font.replace('+',' ')">{{font.replace('+',' ')}}</option>
    ...
    <transition name="fade">
        <moka-modal
            size="lg"
            buttons="none"
            v-if="editor.action === 'addplugin'"
            @close="$action()"
            @click_0="$action()">
            <div slot="title">Plugins</div>
            <div slot="content">
                <moka-editor-plugins @plugin="addPlugin"/>
            </div>
        </moka-modal>
    </transition>
    ...
    <transition name="fade">
        <moka-modal
            buttons="close"
            v-if="editor.action === 'pluginsettings'"
            @close="$action()"
            @click_0="$action()">
            <div slot="title">{{ editor.current.plugin.name }} Settings</div>
            <div class="flex flex-col" slot="content">
                
                <template v-for="field in Object.keys(editor.current.plugin.settings)">
                    <label class="capitalize">{{editor.current.plugin.settings[field].label}}</label>
                    <input type="text" v-if="typeof editor.current.plugin.settings[field].value !='boolean'" v-model="editor.current.plugin.settings[field].value"/>
                    <div class="text-xs">{{editor.current.plugin.settings[field].tip}}</div>
                </template>
            </div>
        </moka-modal>
    </transition>
...
export default {
    ...
    methods:{
        ...
        checkAutosave(){
            ...
            console.log ( 'elapsed time => ' , time - lastSave )
            lastSave ?
