<template>
    ...
    <div :data="getSlider" class="z-top border bg-white">
        ...
        <div class="bg-gray-200 p-1 flex flex-row items-center" v-if="sliderTools && !$attrs.slide">
            SLIDER <i title="Slider settings" class="material-icons mx-2" @click="$store.dispatch('setCurrent',doc),$action('slidersettings')">settings</i> <button class="danger" @click="$emit('remove')">Remove</button>
