...
<div v-if="current">
     <div class="fixed bottom-0 z-top left-0 cursor-pointer border bg-gray-800 w-full transform flex flex-row px-2 h-10 mb-10 items-center">
        ...
        <div v-if="opened && editor.current.tag != 'document'" class="bg-gray-800 flex flex-row w-full">
            ...
            <i v-if="current.type === 'flex'" class="material-icons text-gray-400 hover:text-blue-400 ml-6" @click="$action('addcomponent')" title="Add block/element">add</i>
            

            <i v-if="current.type != 'grid'  && current.type != 'flex'" class="material-icons text-gray-400 hover:text-blue-400 ml-6" @click="$emit('edit',current),$action('edit')" title="Edit content">edit</i>


            <i class="material-icons text-gray-400 hover:text-blue-400 ml-6" @click="$emit('customize'),$action('customize')" title="Edit component">brush</i>
            
            
            <i v-if="current.type==='grid' || current.type === 'flex'" class="material-icons text-gray-400 hover:text-blue-400 ml-6" title="Add reusable block" @click="$action('addreusable')">widgets</i> 
            
            <i v-if="current.type === 'flex'" class="material-icons text-gray-400 hover:text-blue-400 ml-6" @click="$action('addplugin')" title="Add plugin">settings_input_component</i>
