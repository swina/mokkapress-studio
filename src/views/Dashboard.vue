<template>
    <div class="flex flex-row w-full" :data="init">
        ...
        <!-- DASHBOARD MAIN MENU -->
        <nav class="w-2/12 fixed min-h-screen bg-gray-800 text-gray-400 flex flex-col cursor-pointer">
            ...
            <template v-if="menu && menu.items" v-for="item in menu.items">
                 <div :key="item.label" v-if="item.component" :class="'flex flex-row items-center py-1 text-xs p-2 hover:bg-gray-700 font-hairline text-sm ' + activeItem(item.component)" @click="section=item.component,label=item.label,filter=item.filter,$store.dispatch('dashboard_filter','')">
            ...
            <!-- LOGOUT -->
            <div class="flex flex-row items-center py-1 font-thin text-sm p-2 hover:bg-gray-700" @click="closeMe()">
...
import MokaShop from '@/components/shop/settings/settings'
import MokaProducts from '@/components/shop/products/products'
import MokaPlugins from '@/components/plugins/plugins'
...
export default {
    name: 'MokaDashboard',
    components: {
        MokaList,  MokaArticles, MokaCategories, MokaMedia,  MokaHome , MokaSettings, MokaStage, MokaDeploy , MokaGrid , MokaBuild, MokaThemes, MokaProducts , MokaShop, MokaPlugins
    },
    data:()=>({
        label: '',
        section: '',
